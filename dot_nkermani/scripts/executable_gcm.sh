...
Output ONLY the commit message, nothing else. No explanation, no markdown, just the raw commit message.

Changes:
$full_diff"

msg=$(opencode run "$prompt" 2>/dev/null | grep -v "^[[:space:]]*$" | tail -n +2)
